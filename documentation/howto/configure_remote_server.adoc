...
.. In the drop-down menu, select the appropriate host type in the `Host` field. The default host is `Local`. If required, use the `New Host` button to create a new host.
